...
package org.apache.mahout.shell.util
...
class SparkContext(
  ...
  private[shell] val conf = config.clone()
  ...
  private[shell] val env = SparkEnv.create(
    conf,
    "<driver>",
  ...
  private[shell] val addedFiles = HashMap[String, Long]()
  private[shell] val addedJars = HashMap[String, Long]()

  // Keeps track of all persisted RDDs
  private[shell] val persistentRdds = new TimeStampedHashMap[Int, RDD[_]]
  private[shell] val metadataCleaner =
    new MetadataCleaner(MetadataCleanerType.SPARK_CONTEXT, this.cleanup, conf)

  // Initialize the Spark UI
  private[shell] val ui = new SparkUI(this)
  ui.bind()
  ...
  private[shell] val executorMemory = conf.getOption("spark.executor.memory")
  ...
  private[shell] val executorEnvs = HashMap[String, String]()
  ...
  private[shell] var taskScheduler = SparkContext.createTaskScheduler(this, master, appName)
  taskScheduler.start()

  @volatile private[shell] var dagScheduler = new DAGScheduler(taskScheduler)
  dagScheduler.start()
  ...
  private[shell] var checkpointDir: Option[String] = None
  ...
  private[shell] def getLocalProperties: Properties = localProperties.get()

  private[shell] def setLocalProperties(props: Properties) {
  ...
  protected[shell] def checkpointFile[T: ClassTag](
      path: String
    ): RDD[T] = {
   ...
   */
  private [shell] def getPreferredLocs(rdd: RDD[_], partition: Int): Seq[TaskLocation] = {
   ...
   */
  private[shell] def getSparkHome(): Option[String] = {
  ...
  private[shell] def getCallSite(): String = {
   ...
   */
  private[shell] def clean[F <: AnyRef](f: F): F = {
  ...
  private[shell] def newShuffleId(): Int = nextShuffleId.getAndIncrement()

  private val nextRddId = new AtomicInteger(0)

  /** Register a new RDD, returning its RDD ID */
  private[shell] def newRddId(): Int = nextRddId.getAndIncrement()

  /** Called by MetadataCleaner to clean up the persistentRdds map periodically */
  private[shell] def cleanup(cleanupTime: Long) {
...
object SparkContext {
  ...
  private[shell] val SPARK_JOB_DESCRIPTION = "spark.job.description"

  private[shell] val SPARK_JOB_GROUP_ID = "spark.jobGroup.id"

  private[shell] val SPARK_UNKNOWN_USER = "<unknown>"
 ...
 */
private[shell] class WritableConverter[T](
